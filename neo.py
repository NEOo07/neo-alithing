...
@is_option
def IP_Track():
    ip = input(f"{Wh}\n  IP دابنێ : {Gr}")  # INPUT IP ADDRESS
    print()
    print(f' {Wh}============= {Gr}زانیاری پیشان بدە ناونیشانی IP{Wh}=============')
...
def phoneGW():
    User_phone = input(
        f"\n {Wh} ژمارەی تەلەفۆن دابنێ {Gr}Ex [+964xxxxxxxxx] {Wh}: {Gr}")  # INPUT NUMBER PHONE
    ...
    print(f"\n {Wh}========== {Gr}ژمارەی تەلەفۆنی زانیاری پیشان بدە {Wh}==========")
    ...
    try:
        ...
        for site in social_media:
            ...
            if response.status_code == 200:
                ...
                results[site['name']] = (f"{Ye}ناوی بەکارهێنەر نەدۆزرایەوە {Ye}!")
    except Exception as e:
        print(f"{Re}Error : {e}")
        return

    print(f"\n {Wh}========== {Gr}ناوی بەکارهێنەری زانیاری پیشان بدە {Wh}==========")
    print()
...
def showIP():
    ...
    print(f"\n {Wh}========== {Gr}زانیاریەکان IP ی خۆت پیشان بدە {Wh}==========")
...
def call_option(opt):
    if not is_in_options(opt):
        raise ValueError('بژاردە نەدۆزراوەتەوە')
    for option in options:
        if option['num'] == opt:
            if 'func' in option:
                option['func']()
            else:
                print('هیچ کارایییەک دەستنیشان نەکراوە')


def execute_option(opt):
    try:
        call_option(opt)
        input(f'\n{Wh}[ {Gr}+ {Wh}] {Gr}بۆ بەردەوام بوون enter بکە')
        main()
...
╚═╝░░╚══╝╚══════╝░╚════╝░
              ...
              {Wh}[ + ]  C O D E   B Y NEO_O07 Telegram  [ + ]
    """)
...
def main():
    ...
    try:
        ...
        print(f'\n{Wh}[ {Re}! {Wh}] {Re}تکایە ژمارە بنووسە')
        time.sleep(2)
        main()
